...
public class EmployeeLoginController {
    ...
    public void loginBtn() {
        String phone = emp_username.getText();
        String password = emp_password.getText();

        if (phone.isEmpty() || password.isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "Vui lòng nhập số điện thoại và mật khẩu");
            return;
        }

        String query = "SELECT user_id, phone FROM users WHERE phone = ? AND password = ? AND role = 'employee'";
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            if (rs.next()) {
                ...
                showEmployeeDashboard(userId, phone); // Chuyển hướng sau khi đăng nhập thành công
            } else {
                showAlert(Alert.AlertType.ERROR, "Sai số điện thoại hoặc mật khẩu");
            }
        } catch (Exception e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Lỗi kết nối database");
        }
    }

    private void showEmployeeDashboard(int userId, String phone) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/view/user-main-app.fxml"));
            
            // Khởi tạo StoreController với store và cart
            Store_CoffeeShop store = new Store_CoffeeShop();
            ...
            // Sau khi load mới set dữ liệu người dùng
            storeController.setUserData(userId, phone);

            Stage stage = (Stage) emp_loginBtn.getScene().getWindow();
            stage.setScene(new Scene(root));
            stage.centerOnScreen();
        } catch (Exception e) {
            showAlert(Alert.AlertType.ERROR, "Lỗi khi tải giao diện: " + e.getMessage());
            e.printStackTrace();
        }
    }
