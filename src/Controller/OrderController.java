...
public class OrderController implements Initializable {
    ...
    private void handleFilter() {
        try {
            ...
            if (filteredOrders.isEmpty()) {
                showAlert("Không tìm thấy đơn hàng nào. Vui lòng kiểm tra:\n" +
                         "1. Cơ sở dữ liệu đã được khởi tạo\n" +
                         "2. Có đơn hàng trong hệ thống\n" +
                         "3. Bộ lọc không quá hạn chế", Alert.AlertType.INFORMATION);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    // Method to create sample orders if database is empty
    @FXML
    private void createSampleOrders() {
        try {
            ...
            String checkSql = "SELECT COUNT(*) FROM Orders";
            try (PreparedStatement checkStmt = connect.prepareStatement(checkSql);
                 ResultSet rs = checkStmt.executeQuery()) {
                if (rs.next() && rs.getInt(1) > 0) {
                    showAlert("Đã có đơn hàng trong hệ thống!", Alert.AlertType.INFORMATION);
                    return;
                }
            }
            
            // Create sample orders
            String insertOrderSql = "INSERT INTO Orders (customer_id, employee_id, date, status, payment_method, payment_status, total, note, delivery_address) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
            
            // Sample order 1
            try (PreparedStatement stmt = connect.prepareStatement(insertOrderSql)) {
                stmt.setInt(1, 1); // customer_id
                stmt.setInt(2, 3); // employee_id
                stmt.setTimestamp(3, new Timestamp(System.currentTimeMillis()));
                stmt.setString(4, "confirmed");
                stmt.setString(5, "cash");
                stmt.setString(6, "paid");
                stmt.setLong(7, 85000);
                stmt.setString(8, "Đơn hàng mẫu 1");
                stmt.setString(9, "123 Test Street");
                stmt.executeUpdate();
            }
            
            // Sample order 2
            try (PreparedStatement stmt = connect.prepareStatement(insertOrderSql)) {
                stmt.setInt(1, 4); // customer_id
                stmt.setInt(2, 5); // employee_id
                stmt.setTimestamp(3, new Timestamp(System.currentTimeMillis() - 3600000)); // 1 hour ago
                stmt.setString(4, "pending");
                stmt.setString(5, "bank");
                stmt.setString(6, "pending");
                stmt.setLong(7, 120000);
                stmt.setString(8, "Đơn hàng mẫu 2");
                stmt.setString(9, "456 Demo Avenue");
                stmt.executeUpdate();
            }
            
            showAlert("Đã tạo 2 đơn hàng mẫu thành công!", Alert.AlertType.INFORMATION);
            handleFilter(); // Refresh the table
            
        } catch (Exception e) {
            e.printStackTrace();
            showAlert("Lỗi tạo đơn hàng mẫu: " + e.getMessage(), Alert.AlertType.ERROR);
        }
    }
    ...
    private void updateStatistics(ObservableList<Order> orders) {
        ...
        long totalRevenue = (long) orders.stream()
            .filter(order -> !"cancelled".equals(order.getStatus().toString().toLowerCase()))
